...
> **15 products x 3 channels x 3 hook types = 90 hooks per pipeline run**
...
```bash
# Generate hooks for all 15 products x 3 channels (90 hooks)
...
| `products.json` | 15 real Quince products sourced from quince.com (9 categories) |
...
| File | Generated By |
|------|-------------|
| `generated_hooks.json` | `hook_generator.py` — 135 hooks (15 products x 3 channels x 3 types) |
...
- **135 hooks** generated (15 products x 3 channels x 3 types) in ~150s
- **126/135 passed** validation — 9 failures all from same pattern: Claude rounding `.90` prices to whole numbers (`$99.90` → `$99`, `$169.90` → `$170`)
- **0.999 corpus uniqueness** across 945 pairwise comparisons — 1 near-duplicate correctly flagged
